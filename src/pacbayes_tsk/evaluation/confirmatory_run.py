...
def verify_temporal_freeze(
    ...
    if manifest.get("confirmatory_status") != "reserved_not_run":
        raise ConfirmatoryRunError(
            "Temporal freeze does not reserve a confirmatory run."
        )
    return manifest
    ...
    *,
    ...
    for dataset, group in synthetic.groupby("dataset"):
        ...
        if seeds != list(range(1000, 1020)):
            raise ConfirmatoryRunError(
                f"{dataset}: synthetic seeds are not 1000 through 1019."
            )
